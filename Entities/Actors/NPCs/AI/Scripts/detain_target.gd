@tool
extends ActionLeaf

func tick(actor: Node, blackboard: Blackboard) -> int:
  var target: Actor = blackboard.get_value("data").target

  actor.carry_prop.set_texture(target.name)
  actor.carry_prop.show_sprite()
  target.visible = false

  if target is Player:
    target.remove_child(target.remote_transform)
    actor.add_child(target.remote_transform)
  else:
    var target_npc = target as NPC
    target_npc.evaluation_timer.stop()
    target_npc.executor.set_enable(false)

  WorldState.set_captured(target, true)
  return SUCCESS