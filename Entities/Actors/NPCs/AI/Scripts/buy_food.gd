...
func tick(actor: Node, _blackboard: Blackboard) -> int:
	...
	npc.modify_resource(Globals.ResourceType.MONEY, -quantity * 10)
...
func get_quantity(npc):
	var food_resource = npc.get_resource(Globals.ResourceType.FOOD)
	var money_resource = npc.get_resource(Globals.ResourceType.MONEY)
