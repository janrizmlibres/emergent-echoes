...
var action_candidates = []

func evaluation_action(social_practice: Globals.SocialPractice) -> Array:
...
func get_actor_candidates(resource_type: Globals.ResourceType) -> Array[Actor]:
	...
	for actor in peer_actors:
		...
		if not actor.has_resource(resource_type): continue
...
func add_action(action: Globals.Action, resource_type: Globals.ResourceType, data = []) -> void:
	...
	var action_item = [action, data]
	action_candidates.append([action_item, action_weight]);
...
func calculate_weight(resource_type: Globals.ResourceType) -> float:
	...
	var deficiency_point = chosen_resource.get_deficiency_point(weight)
